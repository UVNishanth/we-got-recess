        ...
        static class ZooLunchService extends ZooLunchGrpc.ZooLunchImplBase {
            ...
            Watcher lunchtimeWatcher = e -> {
                if (e.getType() == Watcher.Event.EventType.NodeCreated) {
                    ...
                    String path = lunchpath + "/zk-" + name + "@" + serverPort;
...
//                    default -> checkLeader();
            ...
            AsyncCallback.ChildrenCallback getLunchChildrenCallback = (rc, path, ctx, children) -> {
                switch (KeeperException.Code.get(rc)) {
                    ...
                    case OK -> {
                        ...
                        children.removeIf(znode -> !znode.startsWith("zk-"));
                        lunchmates = children;
            ...
            void addMeCozMNew() {
                zk.create(lunchpath + "/employee/zk-" + name + "@" + serverPort, serverInfo.getBytes(), ZooDefs.Ids.OPEN_ACL_UNSAFE, CreateMode.EPHEMERAL, newEmployeeCallBack, null);
            ...
            void regForLunch() {
                ...
                try {
                    zk.exists(lunchpath + "/readyforlunch", readyforlunchWatcher);
            ...
            void addMeToLunchRoster() {
                zk.create(lunchpath + "/zk-" + name + "@" + serverPort, serverInfo.getBytes(), ZooDefs.Ids.OPEN_ACL_UNSAFE, CreateMode.EPHEMERAL, addToLunchRosterCallback, null);
            }
